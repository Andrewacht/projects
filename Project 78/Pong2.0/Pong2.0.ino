...
int ballOldX = screenWidth/2;
int ballOldY = screenHeight/2;
...
int paddleOneX = 2;
int paddleOneY = screenHeight/2-4;
int paddleTwoX = screenWidth-2;
int paddleTwoY = screenHeight/2-4;
...
void ballMovement() {
  ...
  if (ballDirectionX.equals("left")) {
    ...
    if (ballNewX <= (paddleOneX + paddleWidth) && (ballNewY >= paddleOneY - 2) && (ballNewY < (paddleOneY + paddleHeight))) {
      ballDirectionX = "right";
      if (ballNewY == paddleOneY) {
        currentBallYSpeed = 0;
        ballDirectionY = "up";
      }
      else if (ballNewY == paddleOneY + 6) {
        currentBallYSpeed = 0;
        ballDirectionY = "down";
      }
      else if (ballNewY == paddleOneY + 2) {
        currentBallYSpeed = 0;
        ballDirectionY = "up";
      }
      else if (ballNewY == paddleOneY + 4) {
        currentBallYSpeed = 0;
        ballDirectionY = "down";
      }
      ballTouches++;
    ...
    if (ballNewX >= paddleTwoX - ballWidth && (ballNewY >= paddleTwoY) && (ballNewY < (paddleTwoY + paddleHeight))) {
      ...
      if (ballNewY < screenHeight-4 && ballDirectionY.equals("down")) {
...
void paddleMovement() {
  ...
  if (playerOneY > 800) {
    Serial.println("down");
    if (newPaddleOneY < (screenHeight-2 - paddleHeight)) {
      newPaddleOneY = paddleOneY + 1;
      matrix.fillRect(paddleOneX, newPaddleOneY, paddleWidth, paddleHeight, matrix.Color333(7, 0, 0));
  ...
  if (playerTwoY > 800) {
    Serial.println("down");
    if (newPaddleTwoY < (screenHeight-2 - paddleHeight)) {
      newPaddleTwoY = paddleTwoY + 1;
      matrix.fillRect(paddleTwoX, newPaddleTwoY, paddleWidth, paddleHeight, matrix.Color333(7, 0, 0));
